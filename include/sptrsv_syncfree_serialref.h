                                   ...
                                   val_type    *x)
{
    ...
    val_type *left_sum = (val_type *)malloc(sizeof(val_type) * m * rhs);
    memset(left_sum, 0, sizeof(val_type) * m * rhs);
    ...
    if (substitution == SUBSTITUTION_FORWARD)
    {
        for (int i = 0; i < n; i++)
        {
            ...
            for (int k = 0; k < rhs; k++)
            {
                val_type xi = (b[i * rhs + k] - left_sum[i * rhs + k]) / cscVal[cscColPtr[i]];
                x[i * rhs + k] = xi;
            }
    ...
    else if (substitution == SUBSTITUTION_BACKWARD)
    {
        for (int i = n-1; i >= 0; i--)
        {
            ...
            for (int k = 0; k < rhs; k++)
            {
                val_type xi = (b[i * rhs + k] - left_sum[i * rhs + k]) / cscVal[cscColPtr[i+1]-1];
                x[i * rhs + k] = xi;
            }
                              ...
                              const val_type    *x_ref)
{
    ...
    printf("SpTRSV Serial analyser on L used %4.4f ms\n", time_sptrsv_analyser);
    ...
    printf("SpTRSV Serial executor used %4.4f ms, throughput is %4.4f gflops\n",
    ...
    if (res < accuracy)
        printf("SpTRSV Serial executor passed! |x-xref|/|xref| = %8.4e\n", res);
    else
        printf("SpTRSV Serial executor _NOT_ passed! |x-xref|/|xref| = %8.4e\n", res);
