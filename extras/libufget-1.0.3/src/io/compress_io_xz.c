...
static int xz_open (void **data, const char * path, const uf_io_mode_t mode, _compressed_io_handler * handler);
	...
	if ( f->readonly == 0 ) {
		...
		while ( ret != LZMA_STREAM_END ) {
			...
			if ( ret != LZMA_STREAM_END && ret != LZMA_OK) {
				fprintf(stderr, "LZMA Encode error\n"); 
				break; 
			}
		}
